...
class DMCNN_ARGUMENT(object):
    """
    A DMCNN(Dynamic Multi-Pooling Convolutional Neural) model for argument (pre sentence for input).
    """
    ...
    def __init__(self, sequence_length, num_role, vocab_size, embedding_size, position_size,
                 ...
                 trigger_flags, gpu_flags, bag_word_length, l2_reg_lambda=0.0):
        # initializer
        ...
        self.input_trigger_position = tf.placeholder(tf.int32, [None, sequence_length], name='input_trigger_position')
        self.input_parts_indexs = tf.placeholder(tf.float32, [None, 3, sequence_length], name='inout_parts_indexs')
        ...
        self.input_trigger_adjacent_words = tf.placeholder(tf.int32, [None, 3], name='input_trigger_adjacent')
        ...
        self.scores_lookup_add = tf.placeholder(tf.int32, [None], name='lookup_add')
        ...
        with tf.device(gpu_flags), tf.name_scope('embedding'):
            ...
            with tf.variable_scope('Lexical_FR'):
                ...
                self.lexical_feature_trigger = tf.nn.embedding_lookup(self.word_lookup, self.input_trigger_adjacent_words)
                self.lexical_feature_trigger = tf.reshape(self.lexical_feature_trigger, [-1, 3 * embedding_size])

            with tf.variable_scope('PF'):
                self.PF = tf.nn.embedding_lookup(self.PF_lookup, self.input_word_position)
                self.PF_trigger = tf.nn.embedding_lookup(self.PF_trigger_lookup, self.input_trigger_position)
            ...
            with tf.variable_scope('embedding_all'):
                if trigger_flags == 1:
                    self.embedding_all = tf.concat([self.CWF, self.PF, self.PF_trigger, self.EF], 2)
                else:
        ...
        with tf.device(gpu_flags), tf.name_scope('Convolution-%s' % filter_size):
            # =======================================================================================
            head_inds, middle_inds, tail_inds = tf.unstack(tf.transpose(self.input_parts_indexs, [1, 2, 0]))
            ...
            embedding_all_expand_part2 = self.embedding_all_expand * middle_inds
            embedding_all_expand_part3 = self.embedding_all_expand * tail_inds
            ...
            embedding_all_expand_part3 = tf.transpose(embedding_all_expand_part3, [3, 2, 0, 1])
            # =======================================================================================
            ...
            h3 = tf.nn.relu(tf.nn.bias_add(conv_3, b), name='relu_3')
        ...
        with tf.device(gpu_flags), tf.name_scope('Combine_output_and_dropout'):
            ...
            self.combine_output = tf.concat([self.lexical_feature, self.lexical_feature_trigger, self.h_pool_flat], 1)
        ...
        with tf.device(gpu_flags), tf.name_scope('output'):
            W = tf.get_variable(
                name='W',
                shape=[num_pool_total + 3 * embedding_size + 3 * embedding_size, num_role],
                initializer=initializer
            )
        ...
        with tf.device(gpu_flags), tf.name_scope('split_bag'):
            self.all_scores = tf.reshape(self.scores, [self.bag_num, bag_word_length, num_role])
            # self.all_scores_softmax = tf.reshape(self.scores_softmax, [self.bag_num, bag_word_length, num_role])

        # CalculationMean cross-entropy loss
        with tf.device(gpu_flags), tf.name_scope('loss'):
            self.scores_flat = tf.reshape(self.all_scores, [self.bag_num, -1])
            self.scores_flat_softmax = tf.nn.softmax(self.scores_flat, dim=-1, name="scores_flat_softmax")
            self.scores_max_flat = tf.argmax(self.scores_flat_softmax, 1)
            self.scores_max_score = tf.reduce_max(self.scores_flat_softmax, 1)
            self.row = tf.to_int32(tf.div(tf.to_int32(self.scores_max_flat), num_role))
            self.col = tf.to_int32(tf.mod(tf.to_int32(self.scores_max_flat), num_role))
            # self.scores_max = tf.slice(self.scores, [self.row, 0], [1, num_role])
            self.scores_lookup_ids = tf.add(self.row, self.scores_lookup_add)
            # self.scores_max = tf.nn.embedding_lookup(self.scores_softmax, self.scores_lookup_ids)
            self.scores_max = tf.nn.softmax(tf.nn.embedding_lookup(self.scores, self.scores_lookup_ids))
            self.prediction = tf.arg_max(self.scores_max, 1, name='prediction')
            self.org_label = tf.arg_max(self.input_role, 1)
            self.input_role_trans = tf.transpose(self.input_role)
            losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.scores_max, labels=self.input_role)
