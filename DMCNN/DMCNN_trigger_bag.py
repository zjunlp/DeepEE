...
"""
DMCNN for trigger bag model file
Author: ZUO XINYU
Date: 08/16/7017
"""
...
class DMCNN_TRIGGER_BAG(object):
    """
    A DMCNN(Dynamic Multi-Pooling Convolutional Neural) model for trigger bag (pre sentence for input).
    """
    ...
    def __init__(self, sequence_length, num_role, vocab_size, embedding_size, position_size,
        ...
        self.input_adjacent_words = tf.placeholder(tf.int32, [None, 3], name='input_adjacent_words')
        ...
        with tf.device(gpu_flags), tf.name_scope('loss'):
            ...
            # self.scores_flat_softmax_re = tf.reshape(self.scores_flat_softmax, shape=[self.bag_num, num_role])
            ...
            # self.scores_max = tf.nn.embedding_lookup(self.scores, self.scores_lookup_ids)
